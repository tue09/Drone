...
import copy
import time
from change_array import change_array
from data import number_customer
from data import number_truck
from data import number_drone
from data import speed_truck
from data import speed_drone
from data import capacity_drone
from data import Max_Drone_Flight_Time
from data import Drone_Unloading_Time
from data import depot
from data import customer
from data import truck_path_array
from data import x
...
def point_journey(journey):
    ...
    for l in range(0,capacity_drone):
        ...
        for i in range(0,len(point)-1):
            for j in range(i+1,len(point)):
                if which_truck_package_in(delivery_location(point[j][0]))==which_truck_package_in(delivery_location(point[i][0])):
                    for g in range(0,len(point[j])):
                        point[i].append(point[j][0])
                        point[j].pop(0)
                        if point[j]==[]:
                            point.pop(j)
...
def fitness(drone_journey):
    ...
    for g in range(0,number_customer):
        ...
        for i in range(0,len(point_journey(loading[h])[drone_location[h]-1])):
            for j in range(0,len(decryption_fake[0])):
                for z in range(0,capacity_drone):
                    if decryption_fake[0][j]==delivery_location(point_journey(loading[h])[drone_location[h]-1][i]):
                    ...
                    if drone_location[u]==len(point_journey(loading[u])):
                        ...
                        while time_truck_append[delivery_location(point_journey(loading[u])[drone_location[u]][0])]<9999999:
                            ...
                            for i in range(0,len(point_journey(loading[u])[drone_location[u]-1])):
                                for j in range(0,len(decryption_fake[0])):
                                    for z in range(0,capacity_drone):
                                        if decryption_fake[0][j]==delivery_location(point_journey(loading[u])[drone_location[u]-1][i]):
                                            ...
                                            break
                            if drone_location[u]==len(point_journey(loading[u])):drone_move(u,loading[u])
                            if drone_location[u]==0:
                                break
                    if drone_location[u]==0:
                        loading[u]=[]
    ...
    if index!=len(drone_journey)-1:
        if len(drone_journey[index])!=1:
            ...
            for i in range(len(drone_journey[index])-1,2*(len(drone_journey[index])-1)):
                ...
                if temp[i][index+2]==[]:
                    temp[i].pop(index+2)
    elif index==len(drone_journey)-1:
        if len(drone_journey[index])!=1:
        ...
        if len(temp)!=0:
            ...
            for i in range(0,len(temp)):
                ...
                for j in range(0,len(temp[i])):
                    de=point_journey(temp[i][j])
                    if len(de)!=1:
                        for k in range(0,len(de)):
                            for l in range(1,len(de[k])):
                                if delivery_location(de[k][l])!=delivery_location(de[k][0]):
                                    for rr in range(0,len(decryption_fake[1])):
                                        if delivery_location(decryption_fake[1][rr][0])==delivery_location(de[k][0]):
                                            x1=rr
                                            break
                                    for rr in range(0,len(decryption_fake[1])):
                                        if delivery_location(decryption_fake[1][rr][0])==delivery_location(de[k][l]):
                                            x2=rr
                                            break
                                    decryption_fake[1][x1].append(de[k][l])
                                    decryption_fake[1][x2].remove(de[k][l])
                check[i]=fitness(temp[i])
                garbage[i]=copy.deepcopy(decryption_fake)
            yy=min(check)
            for i in range(0,len(temp)):
                temp[i]=sorted_journey_release_date(temp[i])
                if check[i]==yy:
            ...
            if yy<current_solution:
                ...
                current_solution=yy
                test_check+=1
                decryption=copy.deepcopy(garbage[h])
while number_test_split<=len(drone_journey)-1 or number_test_check<=len(drone_journey)-1:
    number_test_split=0
    number_test_check=0
    ...
    if number_test_split==len(drone_journey)-1 and number_test_check==len(drone_journey)-1:
        break
